package vlog.controller;

import java.io.IOException;
import java.util.List;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import vlog.service.ISharejoinService;
import vlog.service.SharejoinServiceImpl;
import vlog.vo.SharejoinVO;


@WebServlet("/ShareJoin")
public class ShareJoin extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
 
    public ShareJoin() {
        super();
    }

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	
		request.setCharacterEncoding("utf-8");
		
		ISharejoinService service=SharejoinServiceImpl.getService();
		
		List<SharejoinVO> list=service.selectAll();
		
		request.setAttribute("joinlist", list);
		request.getRequestDispatcher("/myPage/nanumpage.jsp").forward(request, response);
	}

	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		doGet(request, response);
	}

}
